...
class BookingList extends Component {
    constructor(props) {
      ...
      handleClickEdit(id) {
        ...
        localStorage.setItem("refundorderId", id);
        navigate("../edit-refund");
      }
      ...
      handleFileChange = (event) => {
        ...
        handleChange(event) {
          ...
          handleSubmit(event) {
            ...
            this.https.post('/searchData',{token:this.token,searchData:this.state.searchData,type:'RefundCarList',vendor_id:this.state.vendor}).then((result)=>{
                let status=result.data.status; 
                if(status === 1){
                    this.setState({
                        carArray: result.data.data
     ...
     render() {
      ...
      const listItems = this.state.carArray.map((val, key) =>{
          ...
          const title1= val.status === 0 ? 'Yes' : 'No'
          const title11= val.status === 0 ? 'Done' : 'Pending'
          ...
          return (
              <tr key={key}>
                  ...
                  <td data-label="Refund Process">{title1}</td>
                  <td data-label="Refund Amount">{val.amount}</td>
                  <td data-label="Recieved By"><td>{val.refunded_by_name}</td></td>
                  <td data-label="Action">
                    &nbsp;&nbsp;
                    <b className={bts1} onClick={this.handleClickEdit.bind(this, val.id)} >{title11}</b></td>
              </tr>
          )
          }
      });
            ...
            <Container>
                    ...
                    <Col md={12}>
                        <div className="breadcromb-box">
                            ...
                            <ul>
                                ...
                                <li> <Link to="/refund-car">Car Refund</Link> </li>
                            </ul>
                        </div>
                    </Col>
                </Row>
                ...
                <div class="page page-profile">
                    ...
                    <Col sm={6} md={8} lg={9}>
                        ...
                        <p class="content-heading">Car Refund</p>
