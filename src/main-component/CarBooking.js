  ...
  handleSubmit(event) {
    ...
    this.https.post('/access_payment', {
      ...
      if (status === 1) {
        ...
        const transaction_id = res.transaction_id; // Get transaction ID from API response
            ...
            try {
              ...
              this.https.post("/razorpay_callback", {
                ...
                if (result.data.status === 1) {
                  const MySwal = withReactContent(Swal);
                  MySwal.fire("Success!", result.data.message, "success");
              ...
              console.log(error);
              const MySwal = withReactContent(Swal);
              MySwal.fire("Error!", "Payment verification failed", "error");
            }
          },
          prefill: {
