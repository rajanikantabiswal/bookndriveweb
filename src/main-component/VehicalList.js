   ...
   constructor(props) {
    ...
    handleClick(id) {
      ...
      handleSubmit(event) {
        ...
        this.https.post('/searchData',{token:this.token,searchData:this.state.searchData,type:'MakeList'}).then((result)=>{
            let status=result.data.status; 
            if(status === 1){
                this.setState({
                    carArray: result.data.data
    ...
    const listItems = this.state.carArray.map((val, key) =>{
        ...
        return (
            <tr key={key}>
                ...
                <td data-label="Car Image" style={{display:'none'}} ><img alt={val.car_name} src={'http://127.0.0.1:8000/public/images/'+val.image} width='60'/> </td>
      ...
      return (
        <Fragment>
            ...
            <div className="page page-profile">
                ...
                <p class="content-heading">MAKE LIST</p>
